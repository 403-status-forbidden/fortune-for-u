...
public class CounselingReservationService {
    ...
    //예약하기
    @Transactional
    public String saveReservation(Long memberId, Long counselorId, String reservationType,
        ...
        Counselor counselor = counselorRepository.findById(counselorId)
                .orElseThrow(EntityNotFoundException::new);
        List<CounselingReservation> reservationList = counselingReservationRepository.findByCounselor(
                counselor);
    ...
    public List<ReservationResponse> getReservation(Long memberId) {
        Member member = memberRepository.findById(memberId)
                .orElseThrow(EntityNotFoundException::new);
        List<CounselingReservation> reservations = counselingReservationRepository.findByMember(
                member);
    ...
    public Long countWaitingList(Long counselorId) {
        Counselor counselor = counselorRepository.findById(counselorId)
                .orElseThrow(EntityNotFoundException::new);
        List<CounselingReservation> reservations = counselingReservationRepository.findByCounselor(
                counselor);
        if (reservations.isEmpty()) {
            return 0L;
        }
        return reservations.stream()
                .filter(reservation -> reservation.getReservationStatus()
    ...
    public List<ReservationResponse> getCoReservation(Long counselorId, String date) {
        ...
        reservations.sort(
                Comparator.comparing(CounselingReservation::getReservationDateTime).reversed());
    ...
    public ReservationResponse checkReservation(Long reservationNo) {
        CounselingReservation counselingReservation = counselingReservationRepository.findById(
                reservationNo).orElseThrow(EntityNotFoundException::new);
